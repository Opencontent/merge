...
function merge_ContentActionHandler( &$module, &$http, &$objectID )
{
    ...
    if ( $http->hasPostVariable( 'MergeNodeSlave' ) )
    {
        $selected_array = array( $http->postVariable( 'MergeNodeSlave' ) );
    }
    ...
    if ( $http->hasPostVariable( 'MergeNodeMaster' ) )
    {
        if ( $merge_node_master != $http->postVariable( 'MergeNodeMaster' ) )
        {
            ...
            $selected_array[] = $merge_node_master;
    ...
    {
        $selected_array = array_unique( $selected_array );
        ...
        if ( !$mnode1 instanceof eZContentObjectTreeNode || !$mnode2 instanceof eZContentObjectTreeNode )
        {
            $module->redirectTo( '/merge/select' );
            return; 
        }
        ...
        $operationResult = eZOperationHandler::execute( 'content',
                                                        'addlocation', array( 'node_id'              => $mobject1->attribute( 'main_node_id' ),
                                                                              'object_id'            => $mobject1->attribute( 'id' ),
                                                                              'select_node_id_array' => $mobject2->attribute( 'parent_nodes' ) ),
                                                        null,
                                                        true );
    
        eZLog::write( var_export($operationResult,1), 'test.log' );
        ...
        switch ( $attribute1->attribute( 'data_type_string' ) )
        {
            case 'ezobjectrelationlist':
                if ( $datamap2[$identifier]->attribute( 'has_content' ) )
                {
                    ...
                    $relations = array_unique( array_merge( $list1, $list2 ) );
                    $attribute1->fromString( implode( '-', $relations ) );
                    $attribute1->store();
                }
                break;
            case 'ezkeyword':
