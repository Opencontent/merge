...
class eZMergeObjectsHandler
{
    protected $masterNode, $slaveNode, $selectedNodeIDs, $translationMap;
    
    function __construct( $masterNodeID, $selectedNodeIDs, $translationMap )
    {
        if ( !empty( $masterNodeID ) )
        {            
            $this->masterNode = eZContentObjectTreeNode::fetch( $masterNodeID );            
        }
        
        if ( !empty( $selectedNodeIDs ) && in_array( $masterNodeID, $selectedNodeIDs ) )
        {
            $this->selectedNodesIDs = $selectedNodeIDs;
            foreach( $selectedNodeIDs as $nodeID )
            {
                if ( $masterNodeID != $nodeID )
                {
                    $this->slaveNode = eZContentObjectTreeNode::fetch( $nodeID );
                    break;
                }
            }
        }
    ...
    {
        return $this->masterNode->attribute( 'url_alias' );
    }
    ...
    protected function sanityCheck()
    {
        ...
        if (  count( $this->selectedNodesIDs ) != 2 )
        {
            throw new Exception( 'Too much nodes selected' );
        }
    ...
    protected function mergeLocations()
    {
        $operationResult = eZOperationHandler::execute( 'content',
                                                        'addlocation',
                                                        array( 'node_id' => $this->masterNode->attribute( 'node_id' ),
    ...
    {        
        ...
        foreach ( $this->translationMap as $language => $nodeID )
        {
            ...
            $this->doContentObjectMerge( $this->masterNode->attribute( 'object' ),
    ...
    {        
        ...
        {            
            ...
            {
                eZContentObjectTreeNodeOperations::move( $child->attribute( 'node_id' ), $this->masterNode->attribute( 'node_id' ) );
            }
        }
    }
    
    protected function doContentObjectMerge( eZContentObject $master, eZContentObject $slave, $language, $useMasterValues )
    {
        ...
        foreach ( $newMasterAttributes as $attribute )
        {
            ...
            if ( isset( $slaveDataMap[$identifier] ) )
            {
                switch ( $attribute->attribute( 'data_type_string' ) )
                {
                    ...
                    {                        
                        $masterList = $attribute->attribute( 'has_content' ) ? explode( '-', $attribute->toString() ) : array();
                        $slaveList = $slaveDataMap[$identifier]->attribute( 'has_content' ) ? explode( '-', $slaveDataMap[$identifier]->toString() ) : array();
        ...
        $operationResult = eZOperationHandler::execute( 'content', 'publish', array( 'object_id' => $master->attribute( 'id' ),
    ...
    protected function updateReverseRelatedObjects()
    {
        ...
        $reverseRelatedList = eZFunctionHandler::execute( 'content', 'reverse_related_objects',
                                                          array( 'object_id' => $slaveObjectID,
                                                                 'all_relations' => true,
        ...
        foreach ( $reverseRelatedList as $id => $reverseRelatedSubList )
        {
            foreach ( $reverseRelatedSubList as $reverseRelatedObject )
            {
                ...
                {                                    
                    ...
                    foreach ( $languageList as $language )
                    {
                        $tmpNode = eZFunctionHandler::execute( 'content', 'node', array( 'node_id' => $mainNodeID,
                            ...
                            if ( empty( $id ) OR $reverseAttribute->attribute( 'contentclassattribute_id' ) == $id )
                            {
                                switch ( $reverseAttribute->attribute( 'data_type_string' ) )
                                {
                                    ...
                                    case 'ezxmltext':
                                        ...
                                        $xml = $oldXml;
                                        $xml = str_ireplace( "object_id=\"$slaveObjectID\"", "object_id=\"$masterObjectID\"", $xml );
                                        $relatedNodeArray = $this->slaveNode->attribute( 'object' )->attribute( 'assigned_nodes' );
                                        $newRelatedNodeID = $this->masterNode->attribute( 'object' )->attribute( 'main_node_id' );
                                        foreach ( $relatedNodeArray as $relatedNode )
                                        {
                                            $relatedNodeID = $relatedNode->attribute( 'node_id' );
                                            $xml = str_ireplace( "node_id=\"$relatedNodeID\"", "node_id=\"$newRelatedNodeID\"", $xml );
                                        }
                                        if ( $xml != $oldXml )
                                        {
                        ...
                        $operationResult = eZOperationHandler::execute( 'content', 'publish', array( 'object_id' => $reverseRelatedObject->attribute( 'id' ),
                                                                                                    'version' => $newVersion->attribute( 'version' ) ) );
                        eZContentCacheManager::clearObjectViewCache( $reverseRelatedObject->attribute( 'id' ), $newVersion->attribute( 'version' ) );
                    }
                }
            }
        }
